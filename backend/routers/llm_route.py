...
async def get_all_learnings():
    ...
    files = [{"FileName": file.split("-")[0], "format": file.split("-")[1].split(".")[0], "is_learned": True} for file in path_files]

    return {"learning-documents":  files}


@router.post("/promp-user/", status_code=status.HTTP_200_OK)
async def prompt_llm(vectore_store_name: str = Form(...), user_input: str = Form(...)):
    """
    Cargar un índice FAISS desde el sistema de archivos y procesar la solicitud
    """
    try:
        # Ruta del archivo FAISS and pikle
        fais_path = f"{vectore_store_name}.index"
        pickle_path = f"{vectore_store_name}.pkl"
        ...
        response = prompt_llm_response(vector_store, user_input=user_input)
        return {"llm_response": response}

    except FileNotFoundError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"El archivo {vectore_store_name}.index no se encontró en el directorio especificado."
        )
